using CarrierRatesQueryV2.Api.Features.Carriers.Enable;
...
namespace CarrierRatesQueryV2.Tests.Features.Carriers.Enable.Unit;

public class EnableCarrierHandlerTests
{
    private static AppDbContext CreateDbContext()
    {
        var options = new DbContextOptionsBuilder<AppDbContext>()
            .UseInMemoryDatabase($"EnableCarrier_{Guid.NewGuid()}")
            .Options;
    ...
    [Fact]
    public async Task HandleAsync_DisabledCarrier_ShouldEnableCarrier()
    {
        ...
        endpoint.Response.IsEnabled.ShouldBe(true);
        endpoint.Response.UpdatedAtUtc.ShouldNotBeNull();
    }

    [Fact]
    public async Task HandleAsync_AlreadyEnabled_ShouldReturnCarrier()
    {
        var db = CreateDbContext();
        var carrierId = Guid.NewGuid();
        db.Carriers.Add(new Carrier
    ...
    public async Task HandleAsync_WithEndpoints_ShouldIncludeEndpoints()
    {
        ...
        endpoint.Response.ShouldNotBeNull();
        endpoint.Response.Endpoints.ShouldNotBeNull();
        endpoint.Response.Endpoints!.Count.ShouldBe(1);
    }
}