    ...
    ICarrierRateStrategyResolver carrierRateStrategyResolver) : Endpoint<Request, Response>
{
    ...
    {
        Post("rates/carrier/slug/{carrierSlug}");
        AllowAnonymous();
    }
    ...
    public override async Task HandleAsync(Request req, CancellationToken ct)
    {
        ...
        var carrier = carriers.FirstOrDefault(c => c.Slug.Equals(req.CarrierSlug, StringComparison.OrdinalIgnoreCase));
...
public sealed class Validator : Validator<Request>
{
    public Validator()
    {
        RuleFor(x => x.CarrierSlug).NotEmpty();
